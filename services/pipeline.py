...
def _write_language_scaffold(project_dir: Path, language: str):
    ...
    if language.lower() == "python":
        ...
        (project_dir / "app.py").write_text('print("Hello from Agent Hub!")', encoding="utf-8")
    elif language.lower() in ("node", "javascript"):
        (project_dir / "package.json").write_text('{"name": "project", "version": "1.0.0", "main": "index.js"}', encoding="utf-8")
        (project_dir / "index.js").write_text('console.log("Hello from Agent Hub!");', encoding="utf-8")
    elif language.lower() in ("react-ts", "react"):
        (project_dir / "package.json").write_text('{"name": "project", "version": "1.0.0", "dependencies": {"react": "^18.0.0"}}', encoding="utf-8")
        (project_dir / "src" / "App.tsx").parent.mkdir(parents=True, exist_ok=True)
        (project_dir / "src" / "App.tsx").write_text('import React from "react";\nexport default function App() { return <div>Hello from Agent Hub!</div>; }', encoding="utf-8")
    else:
        # default python
        (project_dir / "app.py").write_text('print("Hello from Agent Hub!")', encoding="utf-8")
...
async def enhance_prompt(agent: AgentProject, agents: dict) -> bool:
    ...
    try:
        ...
        try:
            ...
            enhanced_content = enhanced_response.get("content", "") if isinstance(enhanced_response, dict) else str(enhanced_response)
            ...
            if "## ENHANCED PROMPT:" in enhanced_content:
                enhanced_prompt = enhanced_content.split("## ENHANCED PROMPT:")[-1].strip()
                ...
                _write_execution_log(agent.project_id, "Prompt enhancement failed, using original")
        except Exception as e:
            # Fallback: use original prompt
            agent.enhanced_prompt = agent.prompt
            agent.execution_log.append(f"Prompt enhancement failed: {e}, using original")
            _write_execution_log(agent.project_id, f"Prompt enhancement failed: {e}, using original")
...
async def generate_code(agent: AgentProject, agents: dict) -> bool:
    ...
    try:
        ...
        file_plan_response = agents["file_planner"].generate_reply(
            messages=[{"role": "user", "content": f"Create a file plan for: {agent.enhanced_prompt}"}]
        )
        file_plan_content = file_plan_response.get("content", "") if isinstance(file_plan_response, dict) else str(file_plan_response)
        ...
        try:
            ...
            if "```json" in file_plan_content:
                ...
                plan_file.write_text(json.dumps(simple_plan, indent=2), encoding="utf-8")
        except Exception as e:
        ...
        code_response = agents["code_generator"].generate_reply(
            messages=[{"role": "user", "content": f"Generate code for: {agent.enhanced_prompt}"}]
        )
        code_content = code_response.get("content", "") if isinstance(code_response, dict) else str(code_response)
            ...
            if project_dir.exists():
                for file_path in project_dir.rglob("*"):
                    if file_path.is_file():
                        ...
                        if (not any(part in rel_path for part in ['.venv', '__pycache__', 'site-packages', 'pip'])):
                            # Include files from nested directories (actual project files)
                            created_files.append(rel_path)
            
            agent.files_created = created_files
            agent.execution_log.append(f"Code generation completed, created {len(created_files)} files")
            _write_execution_log(agent.project_id, f"Code generation completed, created {len(created_files)} files")
            return len(created_files) > 0
        else:
...
""", encoding="utf-8")
    ...
    constraints_file.write_text("""# Project Constraints
...
""", encoding="utf-8")
    ...
    planning_file.write_text("""# Planning Guide
...
async def run_tests(agent: AgentProject, agents: dict) -> bool:
    ...
    try:
        ...
        if language == "python" and (project_dir / "test_*.py").exists():
            result = run_command(["python", "-m", "pytest"], cwd=project_dir, timeout_sec=60)
