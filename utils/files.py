...
def run_python_snippet_in_dir(snippet: str, cwd: Path, timeout_sec: int = 20) -> Dict[str, Any]:
    ...
    proc = subprocess.Popen([sys.executable, temp_file.name], cwd=str(cwd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    try:
...
def extract_python_code_block(text: str) -> Optional[str]:
    ...
    if match:
        ...
        if any(keyword in content for keyword in ['import ', 'def ', 'class ', 'print(', 'if __name__']):
            return content
