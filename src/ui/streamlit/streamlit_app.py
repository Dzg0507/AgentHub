...
try:
    from utils.server_manager import get_server_status, start_server, stop_server, restart_server, is_server_running
...
PIPELINE_STEPS = [
    {"name": "Prompt Enhancement", "status": "pending", "icon": "🎯", "description": "Enhance user prompt with technical requirements"},
    {"name": "File Planning", "status": "pending", "icon": "📋", "description": "Create structured file plan for the project"},
    {"name": "Code Generation", "status": "pending", "icon": "⚡", "description": "Generate complete, runnable code implementation"},
    {"name": "Environment Setup", "status": "pending", "icon": "🔧", "description": "Configure virtual environment and dependencies"},
    {"name": "Code Execution", "status": "pending", "icon": "▶️", "description": "Execute and test the generated code"},
    {"name": "Testing", "status": "pending", "icon": "🧪", "description": "Run comprehensive tests on the implementation"},
    {"name": "Deployment", "status": "pending", "icon": "🚀", "description": "Package and prepare for deployment"},
]
...
def render_pipeline_progress(project: Dict[str, Any]):
    ...
    st.progress(progress, text=f"Progress: {int(progress * 100)}% ({current_step_index}/{len(PIPELINE_STEPS)} steps)")
...
def render_project_card(project: Dict[str, Any]):
    ...
    with st.expander(f"{get_status_icon(status)} Project {project_id[:8]}... - {status.upper()}", expanded=False):
        # Header with key information
        col1, col2, col3 = st.columns([2, 1, 1])
        
        with col1:
            st.markdown(f"**Language:** {language.upper()} | **Guidance:** {guidance.upper()}")
        ...
        with col2:
            ...
            st.markdown(f"<div style='background-color: {status_color}; color: white; padding: 5px; border-radius: 5px; text-align: center;'>{status.upper()}</div>", unsafe_allow_html=True)
        ...
        if execution_log:
            ...
            with log_container:
                for i, log_entry in enumerate(execution_log[-10:]):  # Show last 10 entries
        ...
        with action_cols[0]:
            if st.button("📋 Logs", key=f"logs-{project_id}"):
                ...
                if r.ok:
                    ...
                    norm_logs = [str(x) for x in raw_logs] if isinstance(raw_logs, list) else [str(raw_logs)]
        ...
        with action_cols[3]:
            if st.button("🔧 Setup", key=f"setup-{project_id}"):
                with st.spinner("Setting up project..."):
                    ...
                    if r.ok:
                        setup_data = r.json()
                        st.session_state[f"setup_logs-{project_id}"] = setup_data.get("logs", [])
...
def render_project_creation_form():
    ...
    with st.form("create_project_form"):
        ...
        with st.expander("Advanced Options"):
            ...
            auto_setup = st.checkbox("Auto-setup environment", value=True, help="Automatically create virtual environment and install dependencies")
            auto_test = st.checkbox("Run tests automatically", value=True, help="Automatically run tests after code generation")
            ...
            with st.spinner("Creating your project..."):
                try:
                    ...
                    if r.ok:
                        ...
                        st.markdown("Your project is now being processed by our AI agents. You can monitor its progress in the Projects tab.")
                        
                        # Auto-refresh to show the new project
                        st.rerun()
                    else:
                        st.error(f"❌ Failed to create project: {r.status_code} {r.text}")
                except requests.exceptions.RequestException as e:
...
def render_server_status():
    ...
    try:
        ...
        if r.ok:
            ...
            with col1:
                st.metric("API Status", health.get("status", "Unknown"), "🟢" if health.get("status") == "healthy" else "🔴")
...
def render_debug_tools():
    ...
    st.markdown("Test the agent pipeline with a simple prompt to verify everything is working.")
                    ...
                    try:
                        ...
                        if r.ok:
                            ...
                            with col1:
                                ...
                                st.metric("Return Code", result.get("returncode", "N/A"))
...
def main():
    ...
    with st.sidebar:
        ...
        if SERVER_MANAGEMENT_AVAILABLE:
            ...
            if not server_running:
                if st.button("🚀 Start Everything", use_container_width=True, type="primary"):
                    with st.spinner("Starting server..."):
                        result = start_server()
                        if result["success"]:
        ...
        selected_provider = st.selectbox(
            ...
            index=provider_options.index(st.session_state.get("llm_provider", "gemini")),
        ...
        if st.button("💾 Save LLM Settings", use_container_width=True):
            ...
            st.success(f"✅ LLM settings saved: {selected_provider} with {selected_model}")
            st.rerun()
        
        # Display current settings
        if "llm_provider" in st.session_state:
            st.info(f"**Current LLM Settings:** {st.session_state['llm_provider']} - {st.session_state.get('llm_model', 'default')}")
        
        # Auto-refresh settings
        st.markdown("#### 🔄 Auto-Refresh")
        auto_refresh = st.toggle("Enable Auto-Refresh", value=False, help="Automatically refresh project status")
        if auto_refresh:
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["🏠 Dashboard", "📋 Projects", "🏥 Server Status", "🛠️ Debug Tools"])
        ...
        try:
            ...
            if resp.ok:
                ...
                if projects:
                    ...
                    st.info("No projects yet. Create your first project using the form above!")
            else:
        ...
        try:
            ...
            if resp.ok:
                ...
                if projects:
                    ...
                    with col1:
                        status_filter = st.selectbox("Filter by Status", ["All"] + list(set(p.get("status") for p in projects)))
                    
                    with col2:
                        language_filter = st.selectbox("Filter by Language", ["All"] + list(set(p.get("language", "python") for p in projects)))
                    
                    with col3:
                        sort_by = st.selectbox("Sort by", ["Updated (Newest)", "Updated (Oldest)", "Created (Newest)", "Created (Oldest)"])
                    ...
                    if language_filter != "All":
                        filtered_projects = [p for p in filtered_projects if p.get("language", "python") == language_filter]
    ...
    if "last_refresh" in st.session_state and time.time() - st.session_state["last_refresh"] > 300:  # 5 minutes
        # Clean up old session data
        keys_to_remove = [key for key in st.session_state.keys() if key.startswith("logs-") or key.startswith("setup_logs-")]
